...
PROBLEMA = df_vision %>%
  ...
  ungroup() %>% 
  filter(FECHA_CREACION_ULTIMA == FECHA_CREACION) %>% 
  distinct(DNUM_ID_SOLICITUD,DNUM_DOCUMENTO, FECHA_VIGENCIA, FECHA_CREACION_ULTIMA, PREGUNTA_14) %>%
  group_by(DNUM_DOCUMENTO) %>%
  filter(n_distinct(DNUM_ID_SOLICITUD) > 1,n_distinct(FECHA_VIGENCIA) > 1)
...
PROBLEMA %>%
  kbl(
    ...
    format = "html", 
    align = "c", 
    col.names = c("ID Solicitud","DNI", "Fecha de Vigencia", "Última Fecha Creación", "Pregunta 14")
  ) %>%
  kable_styling(
    full_width = TRUE,
    bootstrap_options = c("striped", "hover", "condensed"),
...
# toc()
load(file = "Consumos_socios.RDATA")
...
# Calcular la edad del socio al momento del consumo
...
DIOP %>%
  kbl(
    ...
    col.names = c(
      "Dioptrías",
      "Grupo Etario al momento del consumo",
      "Cantidad Personas",
