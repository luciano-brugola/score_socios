...
Dado que para el análisis de consumos la variable PREGUNTA_31 no resulta relevante, decidimos continuar con el análisis prescindiendo de ella. Sin embargo, nos encontramos con dos casos particulares en los que, si bien comparten la misma última FECHA_CREACION, presentan dos FECHA_VIGENCIA distintas. Como utilizaremos esta última variable para realizar el cruce con los consumos, se definió tomar, en estos casos, la última FECHA_VIGENCIA registrada.
...
AFILTRAR = AFILTRAR %>% select(DNUM_DOCUMENTO, DID_SOCIO, FEC_ALTA, FEC_BAJA, FEC_VIGENCIA_PLAN)
...
Definimos tomar 4 categorías posibles en la búsqueda de DID_SOCIO correlacionado con su ID de Solicitud:

1- ALTA_ANTERIOR_SOLICITUD: La fecha de alta del Socio es anterior a la fecha de vigencia de la Solicitud análisis

2- ALTA_MAS10_DIAS: La fecha de alta del Socio está dentro de los 10 días posteriores a la fecha de vigencia de la Solicitud

3- OK: La fecha de alta del socio coincide con la fecha de vigencia o se encuentra, como máximo, hasta 10 días posteriores

4- No se encuentra ese N° de Documento en la tabla DSOCIOS (socios sin dar de alta)


Para el análisis tomaremos únicamente el punto 3, tal vez en base a definiciones se podría incorporar el punto 2. 
...
SOCIOS$CATEGORIA = ifelse(is.na(SOCIOS$RESTA), "DNI sin DID_SOCIO", ifelse(SOCIOS$RESTA >= 0 & SOCIOS$RESTA <= TOPEDIAS, "OK", ifelse(SOCIOS$RESTA <0, "Alta Anterior Solicitud", "Alta + 10 días")))

# Entonces grafico de barra de 4 categorias y puedo hablar de personas porque tenemos identificados ya 
# un registro por persona


df_stacked <- SOCIOS %>%
  mutate(
    CATEGORIA = ifelse(is.na(CATEGORIA), "NA", CATEGORIA),
...
plot_ly(df_stacked,
  ...
  layout(
    barmode = "stack",
    title = "Categorías de Socios por Dioptrías",
...
Si queremos analizar si se encuentra el DNI en la base de Socios sin importar las fechas de solicitud podríamos agrupar en dos categorías y visualizarlo de la siguiente manera:

```{r}

SOCIOS$CATEGORIA2 = ifelse(SOCIOS$CATEGORIA=="DNI sin DID_SOCIO", "Sin DID Socio", "Con DID Socio")


# Preparar los datos
df_pie <- SOCIOS %>%
  mutate(CATEGORIA2 = ifelse(is.na(CATEGORIA2), "NA", CATEGORIA2)) %>%
  count(CATEGORIA2) %>%
...
plot_ly(
  ...
  layout(
    title = "Distribución de Socios según DID",
    legend = list(
      x = 1,
      y = 0.5,
      yanchor = "middle"
...
# Una vez que mostras esa visualizacion vamos a empezar a trabajar con los did_socio que encontro y dieron OK
# Y eso lo vamos a aperturar por la PREGUNTA_14

PARA_CONSUMO = SOCIOS %>% dplyr::filter(CATEGORIA=="OK")

```


## Análisis en Consumos de Socios coincidentes

Tomando únicamente la categoría 3 "OK" en la cuál coinciden los N° de Documento y la Fecha de Vigencia de la Solicitud con la Fecha de alta nos quedan `r scales::comma(n_distinct(PARA_CONSUMO$DNUM_DOCUMENTO), big.mark = ".", decimal.mark = ",")` personas.
Tomando sus consumos pudimos obtener el siguiente cuadro de análisis, teniendo en cuenta que para la variable de operaciones filtramos por los siguientes DCOD_PRACTICAS:
...
DIOP <- data.frame(
  Dioptrías = unique(PARA_CONSUMO$PREGUNTA_14)
)


# Segunda variable vamos a habler el table de cantidad de personas
PERSONAS = as.data.frame(table(PARA_CONSUMO$PREGUNTA_14))


# Vamos a hacer el left join
DIOP = DIOP %>% left_join(PERSONAS, by=c("Dioptrías"="Var1"))
names(DIOP)[2] <- "Cantidad Personas"
...
# Traemos los consumos
CONSUMOS <- sqlQueryIn(odbcConnect("datamart"), as.character(PARA_CONSUMO$DID_SOCIO), query_consumos_socios_preexistencias)


# Vamos a pegarle el campo que nos interesa a CONSUMOS para ir filtrando
CONSUMOS = CONSUMOS %>% left_join(PARA_CONSUMO[,c("DID_SOCIO","PREGUNTA_14")], by=c("DID_SOCIO"))
# Tiene 390074 lineas


# Filtramos los consumos con OFT en DDES_GRUPO_PRESUPUESTO
CONSUMOS_OFT <- CONSUMOS %>% filter(str_detect(DDES_GRUPO_PRESUPUESTO, "OFT"))


# Tenemos con consumos en general
CONSUMOSGRAL = CONSUMOS %>%
  group_by(PREGUNTA_14) %>%
  summarise(Socios_Distintos = n_distinct(DID_SOCIO)) %>%
  arrange(desc(Socios_Distintos))

# Cruzamos
DIOP = DIOP %>% left_join(CONSUMOSGRAL, by=c("Dioptrías"="PREGUNTA_14"))
...
# Cruzamos
DIOP = DIOP %>% left_join(CONSUMOSOFT, by=c("Dioptrías"="PREGUNTA_14"))
...
# Cruzamos
DIOP = DIOP %>% left_join(CONSUMOSOP, by=c("Dioptrías"="PREGUNTA_14"))
names(DIOP)[5] <- "Personas con Operacion Refractiva"


# % Operado
DIOP$Porcentaje = DIOP[,5] / DIOP[,2]
...
CONSUMOS_OFT_RESUMEN <- CONSUMOS_OFT %>%
  ...
  group_by(DID_SOCIO, PREGUNTA_14) %>%
  summarise(
...
# Cruzamos
DIOP = DIOP %>% left_join(PROMEDIO_X_GRUPO, by=c("Dioptrías"="PREGUNTA_14"))
...
DIOP[[7]] <- paste0("$ ", format(round(DIOP[[7]]), big.mark = ".", decimal.mark = ","))

DIOP[2:4] <- lapply(DIOP[2:4], function(x) format(x, big.mark = ".", decimal.mark = ","))
...
DIOP %>%
  kbl(
    ...
    col.names = c(
      ...
      "Personas con Operación Refractiva",
