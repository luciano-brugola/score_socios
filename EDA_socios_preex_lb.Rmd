...
df_original %>% 
  ...
  dplyr::filter(!is.na(DNUM_DOCUMENTO)) %>% view()
  dplyr::mutate(Multiples_Nombres = ifelse(test = Cant_Nombres > 1,
                                           yes = "SI",
                                           no = "NO")) %>% 
...
vars_categ <- "PREGUNTA_9"

l <- htmltools::tagList()
i <- 1

for (var_for in vars_categ) {
  plot_generico <- cant_dni_variable(df = df_original,var_select = var_for) %>% 
                              ggplot(aes_string(x = var_for,y = "Porc")) +
                              geom_bar(stat = "identity") +
                              geom_text(aes(label = paste0(round(Porc,1),"%\n(",Cant_Socios,"/",n_distinct(df_original$DNUM_DOCUMENTO),")")), 
                                        position = position_stack(vjust = 0.5), 
                                        color = "black",
                                        size = 3.5) +
                              labs(x = NULL,y = NULL,title = paste0("Cantidad de Socios según ",var_for)) +
                              theme_minimal() +
